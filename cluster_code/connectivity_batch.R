...
#for(loci in c(5)){
  for(runi in 1:length(run)){
    ...
    print("***********************************")
    print("                                   ")
    print(paste0(loc[loci]," ",run[runi]))
    print("                                   ")
    print("***********************************")
    print("                                   ")
    par(mfrow=c(1,1))

    pts = read.csv(file.path(filepath, loc[loci] ,
                             # "all_results_"+  region_i + "_" + simtype +  '_pop_5km_run_' + str(repetition) + '.csv'
                             paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".csv")))
    ...
    #plot(unique_PAs, col="white", main="", border="white")#, ylim=c(-25.595,-24), xlim=c())
    #plot(unique_PAs, col="red", main="", add = TRUE, ylim=c(-25.595,-22))
    #plot(PA_buffer, add = TRUE, col = 'blue', ylim=c(-25.595,-22))

    unique_PAs$buffer_area <- unique_PAs$area - st_area(PA_buffer)
    #plot(st_centroid(unique_PAs), cex= (unique_PAs$area)*10000/180)

    centroid_coords = st_coordinates(st_centroid(unique_PAs))

    unique_PAs$x = centroid_coords[,1]
    unique_PAs$y = centroid_coords[,2]
    unique_PAs$ID = 1:nrow(centroid_coords) #rownames(centroid_coords)
    unique_PAs$spp = 1
    st_crs(unique_PAs) = CRS('+proj=longlat +datum=WGS84')

    st_write(unique_PAs, file.path(filepath,
                                   loc[loci] ,
                                   paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".shp")),
             append=FALSE)

    unique_PAs = sf::read_sf(
      dsn = file.path(filepath,loc[loci]) ,
      layer = paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition)
    )

    rl1 = import.shape(filename = paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".shp"),#strsplit( paste0(strsplit(file, "[.]" )[[1]][1],".shp"),"[/]")[[1]][2],
                       path = paste0(file.path(filepath,loc[loci]),"/"),#file.path(filepath, strsplit( paste0(strsplit(file, "[.]" )[[1]][1],".shp"),"[/]")[[1]][1], "/"),
                       species.col = "spp",
                       ID.col = "ID",
                       area.col = "area", dispersal = dispersal_dist,
                       class.landscape=TRUE)

    file.remove(file.path(filepath,loc[loci],
                          paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".shp")))
    file.remove(file.path(filepath,loc[loci],
                          paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".shx")))
    file.remove(file.path(filepath,loc[loci],
                          paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".prj")))
    file.remove(file.path(filepath,loc[loci],
                          paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_run_", repetition,".dbf")))
    ...
    # st_write(unique_PAs, file.path(filepath, loc[loci] ,
    #                                paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_connectivity_",dispersal_dist,".shp"))
    #          , append = FALSE) # file.path(filepath, paste0(strsplit(file, "[.]" )[[1]][1],"_connectivity_",dispersal_dist,".shp"))


    # write.csv(pts, file.path(filepath, loc[loci] ,
    #                          paste0("all_results_",loc[loci],"_",run[runi],"_pop_5km_connectivity_",dispersal_dist,"_run_", repetition,".txt")))
    #file.path(filepath, paste0(strsplit(file, "[.]" )[[1]][1],"_connectivity_",dispersal_dist,".txt")))
